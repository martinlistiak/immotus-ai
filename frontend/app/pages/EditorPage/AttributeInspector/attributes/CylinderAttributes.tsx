...
import type {
  CylinderAttributes as CylinderAttributesType,
  AbstractSyntaxTree,
} from "app/types/scene-ast";

export const CylinderAttributes = () => {
  const { selectedObjects, dispatchScene } = useSceneContext();
  const cylinderObject =
    selectedObjects[0] as AbstractSyntaxTree<CylinderAttributesType>;
  ...
  useEffect(() => {
    if (cylinderObject) {
      setInputValues({
        radiusTop: cylinderObject.attributes.radiusTop,
        radiusBottom: cylinderObject.attributes.radiusBottom,
        height: cylinderObject.attributes.height,
        radialSegments: cylinderObject.attributes.radialSegments || 32,
        heightSegments: cylinderObject.attributes.heightSegments || 1,
        openEnded: cylinderObject.attributes.openEnded || false,
        thetaStart: cylinderObject.attributes.thetaStart || 0,
        thetaLength: cylinderObject.attributes.thetaLength || Math.PI * 2,
      });
    }
  }, [cylinderObject]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-[70px_1fr] justify-end gap-x-1 gap-y-1 text-[11px] items-center">
        ...
        <Input
          type="text"
          value={inputValues.radiusTop}
          onChange={handleInputChange("radiusTop")}
          onBlur={validateInputOnBlur("radiusTop")}
        />
        <div>Bot Radius</div>
        <Input
          type="text"
          value={inputValues.radiusBottom}
          onChange={handleInputChange("radiusBottom")}
          onBlur={validateInputOnBlur("radiusBottom")}
        />
        <div>Height</div>
        <Input
          type="text"
